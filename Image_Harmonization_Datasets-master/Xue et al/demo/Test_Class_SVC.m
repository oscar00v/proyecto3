function Test_Class_SVC(goal)
close all; clc; 
%clear all;

%goal = 'lum'; %'lum', 'cntrst', 'cct', 'sat', 'hue';

disp(sprintf('Test_Class_SVC. Goal = %s\n', goal));
...
%% Load Model
fn = sprintf('model\\%s_Class_SVC.mat', goal);   
load( fn );  % 'minPerFeature',  'rangePerFeature', 'model'

fn = sprintf('test\\%s_Test_Class_SVC.txt', goal);
fid = fopen(fn,'wt');
...
for p=1:nPic
   ...
   for j = [0,1,2]    % background
      for i = [0,1,2]   % foreground
           ...
           [predicted, accuracy, decision_values] = svmpredict(p, X_test, model);
...
%            fn   = sprintf('%s\\%s\\%s\\cls_SVC\\%d_%d.jpg', path, picnm, goal, i, j);
%            imwrite( outI, fn, 'jpg');
...
saveas(h, sprintf('test\\%s_Test_Class_SVC.jpg', goal) );
