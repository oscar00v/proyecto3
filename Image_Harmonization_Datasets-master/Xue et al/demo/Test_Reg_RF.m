function Test_Reg_RF(goal, region)
clc; close all;  
%clear all;
...
disp(sprintf('Test_Reg_RF. Goal = %s, region = %s\n', goal, region));
...
fn = sprintf('test\\%s_%s_Test_Reg_RF.txt', goal, region);
fid = fopen(fn,'wt');
...
%            outI = manipulateFeature(goal, oriI, oriMask, shift_f, 0);
%            fn   = sprintf('%s\\%s\\%s\\reg_RF\\%s_%d_%d.jpg', path, picnm, goal, region, i, j);
%            imwrite( outI, fn, 'jpg');
...
saveas(h, sprintf('test\\%s_%s_Test_Reg_RF.jpg', goal, region) );
